...
describe.skipIf(!hasInsforge)("staff NFC session contract (T026)", () => {
  it(
    "opens session for valid staff tag slug",
    async () => {
      ...
      const result = await openCaptureSession({
        staffTagSlug: STAFF_SLUG.primary,
        clientFingerprint: uniqueFingerprint("session-open"),
      ...
      // Assert remaining TTL from response time (tolerates InsForge round-trip latency).
      const remainingSeconds = differenceInSeconds(
        new Date(result!.expiresAt),
        new Date(),
      );
      expect(remainingSeconds).toBeGreaterThanOrEqual(280);
      expect(remainingSeconds).toBeLessThanOrEqual(300);
    },
    INTEGRATION_TIMEOUT.sessionTtl,
  );
  ...
  it("returns null for invalid or revoked staff tag", async () => {
    ...
    const result = await openCaptureSession({
      ...
      clientFingerprint: uniqueFingerprint("invalid-tag"),
    });
  ...
  it(
    "deduplicates active session within 45s for same tag + fingerprint",
    async () => {
      const { openCaptureSession } = await import(
        "@/lib/staff/open-capture-session"
      );

      const fingerprint = uniqueFingerprint("dedup");
