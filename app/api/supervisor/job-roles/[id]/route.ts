...
import { updateJobRole } from "@/lib/supervisor/org/job-roles";
import { supervisorOrgErrorResponse } from "@/lib/supervisor/org-errors";
import { jobRoleUpdateSchema } from "@/lib/validators/supervisor-org";
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const parsed = jobRoleUpdateSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const item = await updateJobRole(session, id, parsed.data);
