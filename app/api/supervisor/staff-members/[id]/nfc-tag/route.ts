...
import { assignNfcTag } from "@/lib/supervisor/org/staff-members";
import { supervisorOrgErrorResponse } from "@/lib/supervisor/org-errors";
import { nfcTagAssignSchema } from "@/lib/validators/supervisor-org";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const parsed = nfcTagAssignSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await assignNfcTag(session, id, parsed.data);
