# TagMe Platform — Documento técnico para hackathon y sponsors
...
- Backend: InsForge (BaaS Postgres + Auth + RLS + Storage + AI)
...
- Supervisor: administra incidencias, scorecards y organización del equipo.
...
Este documento cubre router por router, feature por feature, y mapea explícitamente la relación
con InsForge para que los dueños de la plataforma vean el uso real de su tecnología.
...
app/                           # Next.js App Router
  ...
  (guest)/                    # Experiencia del huésped
    layout.tsx
    s/[tagSlug]/page.tsx       # Entrada staff simulada para capture
    t/[tagSlug]/page.tsx       # Hub público por tag NFC (Edge runtime)
    capture/[sessionToken]/page.tsx  # Captura feedback/incidente por staff
    capture/room/[tagSlug]/page.tsx  # Captura directa desde habitación/zona
  (staff)/                    # Panel staff
    layout.tsx
    my-scorecard/page.tsx      # Mi scorecard
    reception/                 # Recepción
      layout.tsx
      page.tsx                 # Gestión de estadías
      consolidate/page.tsx     # Consolidación de estancias
...
lib/                           # Core logic (server-safe)
  insforge.ts                 # Cliente genérico InsForge
  ...
  auth/session.ts             # Sesiones extendidas + helpers de autorización
...
scripts/                      # Seeds y migraciones programáticas
tests/                        # Unit, contract y e2e
specs/                        # Product specs legacy (referencia histórica)
```
...
| Feature | InsForge capability usada |
| --- | --- |
| Login y auth | Auth: `signInWithPassword`, `getCurrentUser` |
| Sesión extendida | Auth + DB: `user_profiles`, `staff_members` |
| RLS y seguridad | DB RLS, SECURITY DEFINER SQL helpers |
| Tags y experiencia | DB: `nfc_tags`, `experience_configs`, `venues` |
| Métricas y vistas | DB: `v_touches_daily`, `v_touches_hourly`, `v_destination_breakdown` |
...
- En desarrollo existe bypass por token fijo STAFF_DEV_TOKEN para acelerar demo.
